...
namespace NiobiumStudios
{
	...
	public class DailyRewardsInterface : MonoBehaviour
	{
		...
		public GameObject paneldailyReward, AllRewardParent, Jeep1, jeep2, jeep3,Reward2x;              // Rewards panel
		...
		void Update()
		{
			...
			if (!readyToClaim)
			{
				...
				textTimeDue.text = string.Format("Next Daily Reward in {0}", formattedTs);
			}
        ...
        private void OnClaimPrize(int day)
		{
			...
			if (reward.reward > 1)
			{
                ...
                jeep2.SetActive(false);
                jeep3.SetActive(false);
                ...
                if (day == 5) { jeep2.SetActive(true); }
                if (day == 7) { jeep3.SetActive(true); }
